<?
	session_start();
	if (isset($_POST['user_id']) && isset($_POST['bulim_id']) && isset($_POST['sanadan']) && isset($_POST['sanagacha']) ) {
		$ret = [];
		if($_POST['_csrf']!=$_SESSION['_csrf']){
		    ...
		    $target_dir="../../docs/haqsiztatil/";
		    $y=md5(time());
		    $tip = strtolower(pathinfo($_FILES["file"]["name"],PATHINFO_EXTENSION));
		    $_FILES["file"]["name"]="haqsiztatil_".$y.".".$tip;
		    $target_file=$target_dir.basename($_FILES["file"]["name"]);
			if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file))
				$file=$_FILES["file"]["name"];
			}
 
			$fetch = Functions::addhaqsiztatil($user_id,$braqam,$bulim_id,$sanadan,$sanagacha,$file);
			if ($fetch) {
			$ret += ['xatolik' => "0"];

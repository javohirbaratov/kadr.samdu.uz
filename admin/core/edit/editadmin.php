<?
	session_start();
	if (isset($_GET['familya']) && isset($_GET['ism']) &&isset($_GET['otch']) &&isset($_GET['login']) &&isset($_GET['email']) &&isset($_GET['id']) && isset($_GET['telefon'] ) && isset($_GET['rol'])   && isset($_GET['telegram_id'])  ) {
		$ret = [];
		...
		if($_GET['_csrf']!=$_SESSION['_csrf']){
			...
			$fetch = Functions::editadmin($id,$familya,$ism,$otch,$login,$email,$telefon,$rol,$telegram_id);
			if ($fetch) {
			$ret += ['xatolik' => "0"];

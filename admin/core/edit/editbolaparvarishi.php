<?
	session_start();
	if ( isset($_POST['user_id']) && isset($_POST['bulim_id']) && isset($_POST['lavozim']) && isset($_POST['sanadan']) && isset($_POST['sanagacha']) && isset($_POST['yosh']) && isset($_POST['hujjat']) ) {
		$ret = [];
		if($_POST['_csrf']!=$_SESSION['_csrf']){
			...
			$yosh = filter($_POST['yosh']);
			$hujjat = filter($_POST['hujjat']);
			$sanagacha = filter($_POST['sanagacha']);
			  $file='';
			  if (isset($_FILES['file'])) {
		    $target_dir="../../../docs/bolaparvarishi/";
		    $y=md5(time());
		    $tip = strtolower(pathinfo($_FILES["file"]["name"],PATHINFO_EXTENSION));
		    $_FILES["file"]["name"]="bolaparvarishi_".$y.".".$tip;
		    $target_file=$target_dir.basename($_FILES["file"]["name"]);
			if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file))
				$file=$_FILES["file"]["name"];
			}
			$fetch = Functions::editbolaparvarishi($id,$braqam,$user_id,$bulim_id,$lavozim,$sanadan,$sanagacha,$yosh,$hujjat,$file);
			if ($fetch) {
			$ret += ['xatolik' => "0"];

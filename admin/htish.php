          ...
          <tbody id="tbl">
            <?php 
              ...
              foreach($fetch as $value){
                ...
                if(strlen($value['htish'])>1){
                  $file='<a target="_blank" href="../docs/htish/'.$value['htish'].'">Yuklash</a>';
                }else{
...
function modalinfos(event) {
  ...
  function taxririnfo(id) {
    $.ajax({
      url:"core/edit/edit?table=<?=str_rot13("htish")?>",
        type: 'POST',
        processData: false,
        contentType: false,

            ...
            <?php 
              ...
              foreach($fetch as $value){
                ...
                if(strlen($value['file'])>1){
                  $fayl='<a target="_blank" href="../docs/moddiy/'.$value['file'].'">Yuklash</a>';
                }else{
...
function modalinfos(event) {
  ...
  var infos = new Array();
    ...
    document.getElementById('sana').value=infos[2];
    document.getElementById('buyruq').value=infos[3];
  ...
  function delinfo(id) {
    $('.modal, #modal11').removeClass('display');
        ...
        modal.close();
    $.ajax({
      url:"core/delete/deletemoddiy.php",
      method:"get",
      data:{
        id:id,
  ...
  var infos = new Array();
    ...
    for(var i=0; i< arr.length; i++){
        infos.push(arr[i].innerHTML);
            ...
            loadFile(
                "../docs/shablon/moddiy_javobgarlik.docx",

<?
	...
	if ($_POST['action'] == 'xodimrasm') {
		//print_r($_POST);
		$id = $_POST['id'];
		$rasex = pathinfo($_FILES['rasmcha']['name'], PATHINFO_EXTENSION);
		$rasm = 'R'.md5(time().uniqid()).'.'.$rasex;
		$path = "../bot/uploads/";
		//print("$rasm");

		if (move_uploaded_file($_FILES['rasmcha']['tmp_name'],$path.$rasm)) {


			$sql = mysqli_query($link,"UPDATE `xodimlar` SET `rasm` = '$rasm'  WHERE `xodimlar`.`id` = '$id'");
			if ($sql) {
				//exit();
				?>
				<script>alert('Bajarildi');</script>
			...
			?>
				...
				<script>alert('Yuklanmadi');</script>
				<script> window.history.back();</script>
